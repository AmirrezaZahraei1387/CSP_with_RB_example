...
class RBModel:
    ...
    def __generate_constraints(self):
        """
        generate constraints based on RB model
        """
        constraints = []
...
def run_strategy(func):
    start = time.time()
    print("Possible Solution: ", func)
    end = time.time()
...
def main():
    ...
    if solve_with == "BT":
        run_strategy(csp_instance.backTracking_search())
    elif solve_with == "FC":
        run_strategy(csp_instance.forward_checking())
    elif solve_with == "FLA":
        run_strategy(csp_instance.full_look_ahead())
    else:
